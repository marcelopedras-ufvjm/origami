...
module Origami
  ...
  class HexaString < ::String
    ...
    #
    def initialize(str = '')
      raise TypeError, "Expected type String, received #{str.class}." unless str.is_a?(::String)
    ...
    def self.parse(stream, _parser = nil) # :nodoc:
      ...
      raise InvalidHexaStringObjectError, "Hexadecimal string shall end with a '#{TOKENS.last}' token" if hexa.nil?
    ...
    def value
      decrypt! if is_a?(Encryption::EncryptedString) and !@decrypted
  ...
  class LiteralString < ::String
    ...
    def self.parse(stream, _parser = nil) # :nodoc:
      ...
      raise InvalidLiteralStringObjectError, 'No literal string start token found' unless scanner.skip(@@regexp_open)

      result = ''
      depth = 0
      while depth != 0 or scanner.peek(1) != TOKENS.last
  ...
  class Date < LiteralString # :nodoc:
    ...
    def initialize(year:, month: 1, day: 1, hour: 0, min: 0, sec: 0, utc_offset: 0)
      ...
      date = format('D:%04d%02d%02d%02d%02d%02d', year, month, day, hour, min, sec)
      ...
      if utc_offset == 0
        ...
        date << format("%02d'%02d", off_hours, off_mins)
      end
